...
class EvaluationSectionSerializer(serializers.ModelSerializer):
    id = serializers.CharField(source='evaluation_label', read_only=True)
    content = serializers.SerializerMethodField()
    free_text = serializers.CharField(source='evaluation', read_only=True)
    label = serializers.CharField(source='evaluation_label', read_only=True)
    ...
    def get_content(self, obj):
        return None if obj.is_continuing_education_education_group_year else obj.common_evaluation
