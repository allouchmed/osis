...
def find_by_egy(egy, academic_calendar_type=None):
    queryset = OfferYearCalendar.objects.filter(education_group_year=egy)
...
def search(**kwargs):
    ...
    if 'education_group_year_id' in kwargs:
        queryset = queryset.filter(education_group_year__id=kwargs['education_group_year_id'])
